...
for (n in 1:nrow(files)) {
  setwd("D:/ucr_data_storage/raw_data/nibrs_from_fbi")
  print(files$file[n])
  data <- readLines(files$file[n], warn = FALSE, skipNul = TRUE)
  gc()
  ...
  for (i in 1:nrow(segment_levels)) {
    ...
    if (length(temp) > 0) {
      ...
      write_lines(temp, file = paste0(
        segment_levels$names[i],
        "_", files$year[n], ".txt"
      ))
      if (segment_levels$names[i] %in% "arrestee_segment" & files$year[n] %in% 2015) {
        vermont <- temp[substr(temp, 5, 6) %in% "VT"]
        write_lines(vermont, file = paste0(
          segment_levels$names[i],
          "_", files$year[n], "vermont.txt"
        ))
      }
      if (segment_levels$names[i] %in% "window_arrestee_segment" & files$year[n] %in% 2015) {
        vermont <- temp[substr(temp, 5, 6) %in% "VT"]
        write_lines(vermont, file = paste0(
          segment_levels$names[i],
          "_", files$year[n], "vermont.txt"
    ...
    rm(temp)
    gc()
    message(segment_levels$names[i])
  }
  rm(data)
  gc()
  Sys.sleep(2)
