...
# combine_years(arrest_groups = "simple_crimes_sex")
  ...
  for (arrest_group in arrest_groups) {
    ...
    for (year in 1980:2016) {
      ...
      data <- data.table::rbindlist(list(data, get(file_name_temp)),
                                    ...
                                    fill = TRUE)
      message(year)
      do.call("rm", list(as.name(file_name_temp))); gc(); Sys.sleep(1)
    }
    save_combined_years(data, name_to_save)
  }
}


save_combined_years <- function(data, name_to_save) {
...
get_ASR_yearly <- function() {
  for (year in 1980:2016) {
    ...
    # 1:length(combined_crimes)
    for (i in 1:length(combined_crimes)) {
      Sys.sleep(1)
      arrests <- ASR[ASR$OFFENSE_CODE %in% combined_crimes[[i]], ]
      Sys.sleep(1)

      if (names(combined_crimes)[i] %in% "simple_crimes_sex") {
        arrests <- get_arrest_data(arrests, simple_arrest_sex_categories)
      } else if (names(combined_crimes)[i] %in% "simple_crimes_race") {
        arrests <- get_arrest_data(arrests, simple_arrest_race_categories)
      } else {
...
make_simple_columns <- function(data) {
  ...
  data$tot_male_adult   <- rowSums(data[, grep("^MALE_(18|19|[2-6].|OVER)",
                                               names(data))],
                                   na.rm = TRUE)
  data$tot_female_juv   <- rowSums(data[, grep("^FEMALE_(10|13|15|16|17)",
                                               names(data))],
                                   na.rm = TRUE)
